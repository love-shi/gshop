    ...
    methods: {
      ...
      _initScroll(){
        ...
        this.foodsScroll = new BScroll('.foods-wrapper', {          
          /* 
            // 因为惯性滑动不会触发
            1：会非实时（屏幕滑动超过一定时间后）派发scroll 事件（频率比较慢）
            2：会在屏幕滑动的过程中实时的派发 scroll 事件（频率比较快）
            3：不仅在屏幕滑动的过程中，而且在 momentum 滚动动画运行过程中实时派发 scroll 事件（在滑动的过程中会切换）
           */
          probeType: 2,      
          click: true
        })
